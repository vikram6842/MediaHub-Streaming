...
export async function videosData({ inputPath, outputPath }) {
  ...
  const ffmpegCommand = `ffmpeg -i "${inputPath}" -preset ultrafast -threads 2 -codec:v libx264 -codec:a aac -hls_time 10 -hls_playlist_type vod -hls_segment_filename "${path.dirname(
    outputPath
